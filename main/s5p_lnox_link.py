'''
INPUT:
    - CSV file (linked_cases.csv) which has S5P (TROPOPMI) filenames
        based on images generated by s5p_lnox_plot_tracks.py
    - S5P Look-up table (LUT)
    - "kind" keywords
        - production: file for calculating production efficiency
                      Read "linked_cases.csv"
...
def get_feature(ds_mask, alpha_high):
    ...
    # dist.fit_transform(scd_trop_conv.ravel()[~np.isnan(scd_trop_conv.ravel())], verbose=0)
    # dist.fit_transform(ds_mask['SCD_Trop'].stack(z=['y','x']).dropna(dim='z').values, verbose=0)
...
def calc_lno2vis(scd_no2, scd_no2_norm, ds_mask, ds_amf, threshold, alpha_bkgd, features, crf_min):
    ...
    # Initialize model
    dist = distfit(distr='lognorm', alpha=alpha_bkgd)
    # Find best theoretical distribution for empirical data X
    dist.fit_transform(scd_no2_norm.stack(z=['y', 'x'])[~np.isnan(scd_no2_norm.stack(z=['y', 'x']))].values, verbose=0)
...
def get_overpass(row):
    """Get the overpass time of swath"""
    filename = os.path.join(clean_dir, row['filename'].split('_')[-5][:6], row['filename'])
    ds_tropomi = xr.open_mfdataset(filename, group='S5P').isel(time=0)

    # pick the labeled mask
    ...
    # area = pixel_area(ds_tropomi, row)
    ...
    varnames = ['surface_albedo_nitrogendioxide_window', 'cloud_albedo_crb', 'surface_pressure', 'cloud_pressure_crb',
                ...
                'nitrogendioxide_tropospheric_column', 'air_mass_factor_troposphere', 'SCD_Trop']
    ...
    return t_overpass, ptropo


def integrate_lno2_xr(top, bottom, peak_pressure, peak_width):
    ...
    '''
    return integrate.quad(lambda x: np.exp(-np.power(x - peak_pressure, 2)/(2*np.power(peak_width, 2))),
                          top, bottom)[0]
...
def main():
    if kind == 'production':
        csv_file = 'linked_cases.csv'
    elif kind == 'lifetime':
        csv_file = 'linked_cases_nolightning.csv'
